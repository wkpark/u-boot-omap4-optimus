			...
			unsigned int arg, unsigned int *response)
{
	...
	if (cmd_index == 0x19 || cmd_index == 0x12) {
...
static int mmc_set_clk_data_width(mmc_controller_data *mmc_contr_cur, unsigned char  data_width, unsigned int transfer_clk)
{
	unsigned char err;
	...
	{
		argument = (3<<24) | (185<<16) | (1<<8) | (0<<0);
		err = mmc_send_cmd(mmc_contr_cur->base, MMC_CMD6, argument, resp);
		if (err != 1)
		{
			printf("mmc_set_clk_data_width card speed set error\n");
			return err;
		}
		
		mmc_reg_out(OMAP_HSMMC_SYSCTL(mmc_contr_cur->base),
				(ICE_MASK | DTO_MASK | CEN_MASK),
				(ICE_STOP | DTO_15THDTO | CEN_DISABLE));
		mmc_reg_out(OMAP_HSMMC_SYSCTL(mmc_contr_cur->base),
				ICE_MASK | CLKD_MASK, (2 << CLKD_OFFSET) | ICE_OSCILLATE);
		while ((OMAP_HSMMC_SYSCTL(mmc_contr_cur->base) & ICS_MASK) == ICS_NOTREADY);
		OMAP_HSMMC_SYSCTL(mmc_contr_cur->base) |= CEN_ENABLE;
	}
	...
	if (data_width == 8)
	{
		...
		if (err != 1)
		{
			printf("mmc_set_clk_data_width card data width set error\n");
			return err;
		}

		OMAP_HSMMC_CON(mmc_contr_cur->base) |= DW8_1_8BITMODE;
	}
	else if (data_width == 4)
	{
		...
		OMAP_HSMMC_CON(mmc_contr_cur->base) &= ~(DW8_1_8BITMODE);
		OMAP_HSMMC_HCTL(mmc_contr_cur->base) |= DTW_4_BITMODE;
	}
	else
	{
		OMAP_HSMMC_CON(mmc_contr_cur->base) &= ~(DW8_1_8BITMODE);
	...
	mmc_controller_data *mmc_cont_cur, mmc_card_data *mmc_c, int size)
{
	...
	err = mmc_send_cmd(mmc_cont_cur->base, MMC_CMD35, start_sec, resp);
	if (1 != err)
	{
		return err;
	}
	
	err = mmc_send_cmd(mmc_cont_cur->base, MMC_CMD36, end_sec, resp);
	if (1 != err)
	{
		return err;
	}
	...
	err = mmc_send_cmd(mmc_cont_cur->base, MMC_CMD38,  0x80008000, resp);
	if (1 != err)
	{
		return err;
	}
