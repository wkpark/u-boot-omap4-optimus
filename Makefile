...
cosmo_build_usermode :    unconfig
	@if [ "$(findstring cosmo_build_usermode, $@)" ] ; then \
		echo "#define CONFIG_COSMO_USERMODE 1" >> ./include/config.h ;\
	fi;

cosmo_build_boot_vibrator :    unconfig
	@if [ "$(findstring cosmo_build_boot_vibrator, $@)" ] ; then \
		echo "#define CONFIG_COSMO_BOOT_VIBRATOR 1" >> ./include/config.h ;\
	fi;

cosmo_rev_b_config :    unconfig
	@if [ "$(findstring _rev_b, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_B 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

cosmo_rev_c_config \
cosmo_rev_c_mipi_config :    unconfig
	@if [ "$(findstring _rev_c, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_C 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _c_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

cosmo_rev_d_config \
cosmo_rev_d_mipi_config :    unconfig
	@if [ "$(findstring _rev_d, $@)" ] ; then \
...
cosmo_rev_e_config \
cosmo_rev_e_mipi_config :    unconfig
	@if [ "$(findstring _rev_e, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_E 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _e_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

cosmo_rev_1.0_config \
cosmo_rev_1.0_mipi_config :    unconfig
	@if [ "$(findstring _rev_1.0, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_10 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _1.0_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

cosmo_rev_1.1_config \
cosmo_rev_1.1_mipi_config :    unconfig
	@if [ "$(findstring _rev_1.1, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_11 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _1.1_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

cosmo_rev_1.11_config \
cosmo_rev_1.11_mipi_config :    unconfig
	@if [ "$(findstring _rev_1.1, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_11 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _1.11_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan

#LGE_CHANGE_DOMASTIC - start
cosmo_su760_rev_a_config \
cosmo_su760_rev_a_mipi_config :    unconfig
	@if [ "$(findstring su760, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_C 1" >> ./include/config.h ;\
		echo "#define CONFIG_COSMO_SU760 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _a_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan	

cosmo_su760_rev_b_config \
cosmo_su760_rev_b_mipi_config :    unconfig
	@if [ "$(findstring su760, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_D 1" >> ./include/config.h ;\
		echo "#define CONFIG_COSMO_SU760 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _b_mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;
	@./mkconfig -a cosmopolitan arm omap4 cosmopolitan		

cosmo_su760_rev_d_config \
cosmo_su760_rev_d_mipi_config :    unconfig
	@if [ "$(findstring su760, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_11 1" >> ./include/config.h ;\
		echo "#define CONFIG_COSMO_SU760 1" >> ./include/config.h ;\
	fi;		
...
cx2_evb_mipi_config :    unconfig
	@if [ "$(findstring _evb, $@)" ] ; then \
		echo "#define CONFIG_CX2_EVB 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_a_mipi_config :    unconfig
	@if [ "$(findstring _rev_a, $@)" ] ; then \
		echo "#define CONFIG_CX2_REV_A 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_b_mipi_config :    unconfig
	@if [ "$(findstring _rev_b, $@)" ] ; then \
		echo "#define CONFIG_CX2_REV_B 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_c_mipi_config :    unconfig
	@if [ "$(findstring _rev_c, $@)" ] ; then \
		echo "#define CONFIG_CX2_REV_C 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_d_mipi_config :    unconfig
	@if [ "$(findstring _rev_d, $@)" ] ; then \
		echo "#define CONFIG_CX2_REV_D 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_1.0_mipi_config :    unconfig
	@if [ "$(findstring _rev_1.0, $@)" ] ; then \
		echo "#define CONFIG_CX2_REV_10 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
		echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;		
...
cx2_rev_1.11_config \
cx2_rev_1.11_mipi_config :    unconfig
	@if [ "$(findstring _rev_1.1, $@)" ] ; then \
		echo "#define CONFIG_COSMO_REV_11 1" >> ./include/config.h ;\
	fi;		
	@if [ "$(findstring _mipi, $@)" ] ; then \
...
cx2_su870_rev_a_mipi_config : unconfig
	@if [ "$(findstring su870, $@)" ] ; then \
	        echo "#define CONFIG_CX2_REV_A 1" >> ./include/config.h ;\
		echo "#define CONFIG_CX2_SU870 1" >> ./include/config.h ;\
	fi;
	@if [ "$(findstring _mipi, $@)" ] ; then \
	        echo "#define CONFIG_COSMO_MIPI_HSI 1" >> ./include/config.h ;\
	fi;
